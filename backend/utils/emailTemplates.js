...
function saleConfirmationEmail({ sellerName, clientOrBuyerName, type, paymentType, items, totalAmount, creditTerm }) {
...
function networkSaleEmail({ buyerName, sellerName, items, totalAmount, paymentType, creditTerm }) {
  const paymentLabel = paymentType === 'contado' ? 'Contado' : 'Crédito';
...
function paymentReceivedEmail({ sellerName, amount, totalAmount, paidAmount, remaining }) {
  const body = `
...
function debtSettledEmail({ sellerName, totalAmount, clientOrBuyerName }) {
  const body = `
...
function newNetworkMemberEmail({ sponsorName, newMemberName, newMemberRole }) {
  const body = `
